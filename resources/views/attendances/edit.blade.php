<x-app-layout>
    ...
    <!-- Inisialisasi Alpine.js dengan data dari Database -->
    <div class="py-12 bg-slate-50 dark:bg-gray-900 min-h-screen" 
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <form action="{{ route('attendances.update', $attendance->id) }}" method="POST">
                @csrf
                @method('PUT') <!-- Wajib ada untuk proses Edit/Update -->
                ...
                <div class="bg-white dark:bg-slate-800 rounded-3xl p-8 shadow-sm border border-slate-100 dark:border-slate-700 mb-8">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <!-- Badge Tanggal -->
                        <span class="text-sm font-bold text-slate-500 bg-slate-100 dark:bg-slate-700 dark:text-slate-300 px-3 py-1 rounded-lg">
                            Tanggal: {{ \Carbon\Carbon::parse($attendance->date)->translatedFormat('d F Y') }}
                        </span>
                    </div>
                    ...
                    <div x-show="status === 'hadir'" x-transition class="mt-6 grid grid-cols-2 gap-6 p-6 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-200 dark:border-slate-700">
                        <div>
                            ...
                            <input type="time" name="check_in" value="{{ $attendance->check_in }}" class="w-full rounded-xl border-slate-300 focus:border-blue-500 focus:ring-blue-500 dark:bg-slate-800 dark:border-slate-600 dark:text-white">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-500 uppercase mb-2">Jam Pulang</label>
                            <input type="time" name="check_out" value="{{ $attendance->check_out }}" class="w-full rounded-xl border-slate-300 focus:border-blue-500 focus:ring-blue-500 dark:bg-slate-800 dark:border-slate-600 dark:text-white">
                        </div>
                    </div>
                    ...
                    <div x-show="status !== 'hadir'" x-transition class="mt-6">
                        ...
                        <textarea name="notes" rows="3" placeholder="Jelaskan alasan izin atau sakit..." class="w-full rounded-xl border-slate-300 focus:border-blue-500 focus:ring-blue-500 dark:bg-slate-800 dark:border-slate-600 dark:text-white">{{ $attendance->notes }}</textarea>
                    </div>
                </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
                                    ...
                                    <div class="md:col-span-4 space-y-4">
                                        <div>
                                            ...
                                            <select :name="`activities[${index}][type]`" x-model="activity.type" class="w-full rounded-xl border-slate-300 focus:border-blue-500 focus:ring-blue-500 dark:bg-slate-800 dark:border-slate-600 dark:text-white">
                                        ...
                                        <div>
                                            ...
                                            <input type="text" :name="`activities[${index}][title]`" x-model="activity.title" placeholder="Contoh: Belajar Laravel Eloquent" class="w-full rounded-xl border-slate-300 focus:border-blue-500 focus:ring-blue-500 dark:bg-slate-800 dark:border-slate-600 dark:text-white" required>
                                        </div>
                                    </div>
                ...
                <div class="mt-8 flex justify-end gap-4">
                    <a href="{{ route('attendances.index') }}" class="px-6 py-4 rounded-2xl bg-slate-100 text-slate-600 font-bold hover:bg-slate-200 transition">Batal</a>
    ...
    <!-- Alpine JS Logic (Khusus Edit: Menerima Data Awal) -->
    <script>
